...
import { Package, CheckCircle, X, Search, Truck, Banknote, CreditCard, Sparkles } from 'lucide-react';
...
const OrdersTab = memo(({ orders, updateOrderStatus, medicines = [] }) => {
    ...
    // On first render, capture all existing IDs as "already seen"
    useEffect(() => {
    ...
    // When orders list changes, detect genuinely new ones
    useEffect(() => {
        if (seenOrderIds.current === null) return;
        const incoming = orders.filter(o => !seenOrderIds.current.has(o.id));
        if (incoming.length > 0) {
            ...
            // Auto-clear the "new" highlight after 30 seconds
                                    ...
                                    return (
                                        <motion.tr
                                            ...
                                            style={isNew ? {
                                                ...
                                                boxShadow: 'inset 0 0 0 2px #10b981, 0 4px 20px rgba(16,185,129,0.12)',
                                                position: 'relative'
                                            } : {}}
                                        >
                                            <td className="customer-info-cell">
                                                ...
                                                {order.message && (
                                                    <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: '#0984e3', fontStyle: 'italic', background: '#eff6ff', padding: '4px 8px', borderRadius: '6px' }}>
                                                        "{order.message}"
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td className="nowrap-cell">
                                                <div className="action-btns-premium">
                                                    {order.status === 'Pending' && (
                                                        <>
                                                            <button
                                                                ...
                                                                onClick={() => {
                                                                    const pMethod = order.payment_method || order.paymentMethod || 'COD';
                                                                    const isOnline = pMethod.toUpperCase() === 'ONLINE';
                                                                    updateOrderStatus(order.id, isOnline ? 'Paid & Proceeded' : 'Confirmed');
                                                                }}
                                                                title="Confirm Order"
                                                            >
                                                                <CheckCircle size={20} />
                                                    ...
                                                    {(order.status === 'Confirmed' || order.status === 'Paid & Proceeded') && (
                                                        <button
                                                            className="btn-action-premium confirm"
